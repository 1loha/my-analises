...
class SQLDataBase(object):
    ...
    def __init__(self, pathName):
        ...
        with sq.connect(self.path) as con:
            ...
            self.cur = con.cursor()
            #табл расх катег
            self.cur.execute("""CREATE TABLE IF NOT EXIST expenseCat (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL)""")
            #табл доход катег
            self.cur.execute("""CREATE TABLE IF NOT EXIST incomeCat (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL)""")
            #табл общ расходов
            self.cur.execute("""CREATE TABLE IF NOT EXIST expense(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            exp_id INTEGER,
            expDate DATE,
            cash MONEY,
            com TEXT NOT NULL)""")
            #табл общ доходов 
            self.cur.execute("""CREATE TABLE IF NOT EXIST income(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            inc_id INTEGER,
            incDate DATE,
            cash MONEY,
